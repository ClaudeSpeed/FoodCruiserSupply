...
public class CruiseRouteBuilder extends RouteBuilder {
	...
	public void configure() {
		...
		// RSS url
		String rssURL = "https://"
				+ "?pid=12311211&sorter/field=issuekey&sorter/order=DESC&tempMax=1000&delay=10s"; // query
																									// options
																									// for
																									// the
																									// URI
		...
		if (runErrorHandling) {
			errorHandler(deadLetterChannel(
					"foodSupplyCruise-jms:queue:error.queue")
					.useOriginalMessage().maximumRedeliveries(5)
					.onRedelivery(new ErrorProcessor()).redeliveryDelay(5000));
		}
		...
		if (runWeather) {
			from("weather://weatherService?location=Vienna,Austria&mode=JSON")
					...
					.choice()
					.when(header("Rain"))
					.to("file://noRain?fileName=${date:now:yyyy-MM-dd} WeatherData")
					.otherwise()
					.to("file://weather?fileName=${date:now:yyyy-MM-dd} WeatherData");
		}
		...
		if (runMail) {
			...
			Endpoint fromMail = endpoint("imaps://imap."
					+ domain
					+ ":993?username="
					+ username
					+ "&password="
					+ password
					+ "&fetchSize=1&searchTerm.subjectOrBody=OrderNr&unseen=true&consumer.delay=60000");
			Endpoint fromMailPrice = endpoint("imaps://imap."
					+ domain
					+ ":993?username="
					+ username
					+ "&password="
					+ password
					+ "&searchTerm.body=Price&unseen=true&consumer.delay=60000&debugMode=false");
			Endpoint toMail = endpoint("smtps://smtp." + domain
					+ ":465?username=" + username + "&password=" + password);

			// ask for price for order
			from("foodSupplyCruise-jms:queue:orderIn.queue").process(
					new AskPriceProcessor()).to(toMail);

			// receive priced order
			from(fromMailPrice).process(new MyMailProcessor()) // Convert from
																// XML to
																// Object, Set
																// Header
					.aggregate(header("OrderID"), new MyAggregatorStrategy()) // Decide
																				// cheeper
																				// price
																				// //
																				// better
			...
			// send order mail
			from("foodSupplyCruise-jms:queue:mailbuffer.queue").process(
					new ToMailProcessor()).to(toMail);

			// receive confirmMail and translate to Invoice
			from(fromMail).process(new ToInvoiceProcessor()).to(
					"foodSupplyCruise-jms:queue:processedMail.queue");
		}

		// push to twitter: https://twitter.com/Cruise_Food (username:
		// Cruise_Food password: foodSC01)
		if (runTwitter) {
			...
			from("direct:tweet").process(new TwitterProcessor()).to(
		...
		if (runCurrencyConverter) {
			...
			from("foodSupplyCruise-jms:queue:processedMail.queue").process(
					new GenerateReportProcessor()).to(
					"foodSupplyCruise-jms:queue:generatedReports.queue");

			// convert curency
			from("foodSupplyCruise-jms:queue:generatedReports.queue").process(
					new CurrencyConverterProcessor()).to(
					"foodSupplyCruise-jms:queue:convertedReports.queue");
		...
		if (runFtpStore) {
			...
			from("foodSupplyCruise-jms:queue:advancedConvertedReports.queue")
					.process(new ReportToFileProcessor()).to(
							"file://target/upload");
			...
			// store on ftp server
			// to see result, connect to ftp://81.19.145.70:21
			// username=hockeystuff_cruiserfoodsupply pass=cfs1234
			from("file:target/upload?moveFailed=../error")
					.log("Uploading file ${file:name}").to("{{ftp.client}}")
